...
import {PageItem} from '@/app/cms/sdk/dto/page-item';
...
import {ContentListEntity} from '@/app/components/ContentList/content-list-entity';
import {ContentListRestService} from '@/app/components/ContentList/content-list-rest.service';
import {ContentListDetail} from '@/app/components/ContentList/Detail/ContentListDetail';
import {ContentListModelDetail} from '@/app/components/ContentList/Detail/ContentListDetailModel';
import {ContentListMaster} from '@/app/components/ContentList/Master/ContentListMaster';
...
export function ContentList(props: ModelBase<ContentListEntity>) {
    ...
    function handleListView() {
        ...
        let contentListMasterModel: ContentListModelMaster = {
            OnDetailsOpen: (sdkItem) => {
                ...
                if (props.Properties.DetailPageMode === 'SamePage') {
                    ...
                    const newUrl =
                        ...
                        sdkItem.ItemDefaultUrl +
                        window.location.search;
                    window.history.pushState(detailItem, '', newUrl);
                } else if (props.Properties.DetailPage) {
                    RestService.getItem(
                        RestSdkTypes.Pages,
                        props.Properties.DetailPage.ItemIdsOrdered[0],
                        props.Properties.DetailPage.Content[0].Variations[0].Source,
                    ).then((page: SdkItem) => {
            ...
            OpenDetails: !(
                props.Properties.ContentViewDisplayMode === 'Master' && props.Properties.DetailPageMode === 'SamePage'
            ),
    ...
    useEffect(() => {
        if (props.Properties.ContentViewDisplayMode === 'Automatic') {
            ...
            if (
                props.Properties.SelectedItems &&
                props.Properties.SelectedItems.Content &&
                props.Properties.SelectedItems.Content.length > 0
            ) {
                const selectedContent = props.Properties.SelectedItems.Content[0];
                handleDetailView({
...
function getAttributesWithClasses(
    props: ModelBase<ContentListEntity>,
    fieldName: string,
    additiinalClasses: string | null,
    ...
    if (additiinalClasses) classAttribute.Value += ` ${additiinalClasses}`;

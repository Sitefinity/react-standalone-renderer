...
import {GenericContentItem} from '@/app/cms/sdk/dto/generic-content-item';
...
import {ODataFilterSerializer} from '@/app/cms/sdk/services/odata-filter-serializer';

export class RestService {
    public static getItemWithFallback<T extends SdkItem>(itemType: string, id: string, provider: string): Promise<T> {
        const wholeUrl = `${RestService.buildItemBaseUrl(
            itemType,
        )}(${id})/Default.GetItemWithFallback()${RestService.buildQueryParams({
    ...
    public static getItemWithStatus<T extends SdkItem>(
        itemType: string,
        id: string,
        provider: string,
        ...
        const wholeUrl = `${RestService.buildItemBaseUrl(
            itemType,
        )}(${id})/Default.GetItemWithStatus()${RestService.buildQueryParams(queryParamsForMethod)}`;
    ...
    public static getSharedContent(id: string, cultureName: string): Promise<GenericContentItem> {
        ...
        return fetch(
            `${RestService.buildItemBaseUrl(
                RestSdkTypes.GenericContent,
            )}/Default.GetItemById(itemId=${id})${RestService.buildQueryParams(queryParamsForMethod)}`,
        ).then((x) => x.json());
    }
            ...
            if (!fieldMatch && relatedFields.some((x) => x === field)) {
                ...
                if (relatedFields.some((x) => x === fieldName)) {
                    ...
                    if (relatedTypeName) {
                        ...
                        relatedSimpleFields = relatedFieldsInput.filter((x) =>
                            relatedSimpleFields.some((y) => y === x),
                        );

...
import {FilterClause, FilterOperators, StringOperators} from '@/app/cms/sdk/filters/filter-clause';
...
export class ODataFilterSerializer {
    ...
    private serializeCombinedFilter(filter: CombinedFilter, filterContext: FilterContext): string | null {
        ...
        serializedChildFilters = serializedChildFilters
            .map((x) => {
                if (!x) return null;
    ...
    private serializeFilterClause(filter: FilterClause, filterContext: FilterContext): string | null {
        ...
        switch (filter.Operator) {
            ...
            case FilterOperators.DoesNotContain:
                var serializedValues = this.serializeFilterValuesArray(
                    filter.FieldValue,
                    filter.FieldName,
                    filterContext,
                );
                if (serializedValues.length > 0) {
                    var serializedValueAsString = '';
                    if (ServiceMetadata.isPropertyACollection(filterContext.Type, filter.FieldName)) {
                        serializedValueAsString = serializedValues
                            .map((x) => `${this.lambdaVariableName} eq ${x}`)
                            .join(' or ');
                        serializedValueAsString = `${fieldNameWithPrefix}/any(x: ${serializedValueAsString})`;
                    } else {
                        serializedValueAsString = serializedValues
                    ...
                    if (filter.Operator == FilterOperators.DoesNotContain)
                        serializedValueAsString = 'not ' + serializedValueAsString;

                    return serializedValueAsString;
                }

                return null;
            case FilterOperators.ContainsAnd:
                var serializedValues2 = this.serializeFilterValuesArray(
                    filter.FieldValue,
                    filter.FieldName,
                    filterContext,
                ).map(
                    (x) => `${fieldNameWithPrefix}/any(${this.lambdaVariableName}: ${this.lambdaVariableName} eq ${x})`,
                );

                if (serializedValues2.length > 0) {
                    var serializedValueAsString = serializedValues2.join(' and ');
                    return serializedValueAsString;
                }
            ...
            case StringOperators.Contains:
                var serializedValueForString = this.serializeFilterValue(
                    filter.FieldValue,
                    filter.FieldName,
                    filterContext,
                );
            ...
            default:
                throw new Error(
                    `The value provided for the operator filter clause: ${filter.Operator} is not supported`,
                );
        }
    }
